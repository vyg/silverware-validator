    ...
    if ($form.data('client-side')) {
      ...
      new Parsley.Factory($form[0], {
        ...
        errorsContainer: function (el) {
          ...
          return $holder.hasClass('fieldholder-small')
            ? $holder
            : el.$element.closest('.field')
        },
        errorClass: $form.data('field-error-class'),
        successClass: $form.data('field-success-class'),
        errorsWrapper:
          '<div class="' + $form.data('error-wrapper-class') + '"></div>',
      ...
      Parsley.on('field:validated', function (e) {
        ...
        var $group = e.$element.closest(
          '.' + $form.data('group-class').replace(/^\./, '')
        )
